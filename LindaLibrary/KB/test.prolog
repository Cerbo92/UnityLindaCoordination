...
out_d(T) :- assert(T), serveWaitQueue(T).
...
rd_susp(T) :- T.
rd_susp(T) :- \+ T, assert(tuple_s(rd,T,$this)), fail. % $me stands for the GameObject that called this method
...
serveWaitQueue(T) :- loopUntilIN(T,[],L), log(L), \+ L=null, serveAgents(L).
...
%loopUntilIN(T,Acc,L) :- tuple_s(M,T,V), retract(tuple_s(M,T,V)), append(Acc,[tuple_s(M,T,V)],Y), log(M), loopUntilIN(tuple_s(M,T,V),Y,L).% ; loopUntilIN(T,Y,Y).%; retract(tuple_s(M,T,V)), loopUntilIN(T,[Acc|tuple_s(M,T,V)],[Acc|tuple_s(M,T,V)]).

%loopUntilIN(tuple_s(rd,T,V),Acc,L) :- tuple_s(rd,T,V), retract(tuple_s(rd,T,V)), append(Acc,[tuple_s(rd,T,V)],Y), log(Y), loopUntilIN(tuple_s(M,T,V),Y,L).
%loopUntilIN(tuple_s(in,T,V),Acc,L) :- tuple_s(in,T,V), retract(tuple_s(in,T,V)), append(Acc,[tuple_s(in,T,V)],Y), log(Y), loopUntilIN(T,Y,Y).
