...
public class LindaKBTest : AbstractLinda {
...
//		LindaLibrary.SetGlobalKB ("linda");
//		LindaLibrary.SetLocalKB ("knowledgebase2", gameObject);
//		KnowledgeBase myKB = getLocalKB ();
//		print ("BLABLA " + myKB.IsTrue (new Structure ("pinolo")));//pinolo è solo nella KB locale, non nella globale



		//setLocalKB ("linda");
		//await suspensionTest ();
		//print ("DONE SUSPENSION TEST ");
	...
	{
		print ("CIAO1");
		KnowledgeBase k = LindaLibrary.GetGameobjectLocalKB (GameObject.Find ("POI"));
		print ("TESTONE "+ LindaLibrary.Linda_RD ("cicciolina",k));
		print ("TESTSITUATEDKB "+ await LindaLibrary.Linda_RD_SUSP ("pippo(1)",k,this));
		//LindaLibrary.SetLocalKB ("linda",gameObject);
		KnowledgeBase k2 = LindaLibrary.GetGameobjectLocalKB (GameObject.Find ("LindaAgent2"));
		print ("LA1 "+ await LindaLibrary.Linda_RD_SUSP ("pinolo(666)",k2,this));
		print ("TEST1: " + await LindaLibrary.Linda_RD_SUSP ("chop(101)",k2,this));
		//			print ("TEST LOCALKB, TUST BE FALSE: " + LindaLibrary.Linda_RD ("chop(12)",LocalKB));
		//			KnowledgeBase k2 = LindaLibrary.GetGameobjectLocalKB (GameObject.Find ("LindaAgent2"));
		//			print ("TEST GLOBALKB, MUST BE TRUE: " + LindaLibrary.Linda_RD ("chop(1)"));
		//			print ("TRYING UNIFICATION WITH LOWERCASE VARIABLE: " + LindaLibrary.Linda_RD ("xr","chop(Xr)",this));
		//			print ("TEST EXTERNAL KB, MUST BE TRUE: " + await LindaLibrary.Linda_RD_SUSP ("chop(101)",k2));//chop(10) e chop(0) non ci sono, pinolo(666) c'è
		print ("CAZZO1");
		once = true;
	...
	async void Update () {
		print ("UPDATE1");//continuamente chiamato, ovviamente
		while (!once) {
			once = true;
			await Ehi ();
			print ("FINEAWAIT1");//print non appena finisce l'await
		}
	...
	async Task suspensionTest ()
	{

		print ("SUSPEND THIS 1 " + await LindaLibrary.Linda_RD_SUSP ("pasticcio(loves,ciccio)",this));
		print ("DONE SUSPENSION 1");

		print ("SUSPEND THIS 2 " + await LindaLibrary.Linda_RD_SUSP ("pasticcio(loves,ciccio)",this));
		print ("DONE SUSPENSION 2");
	}
}
